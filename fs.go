...
func dirList(ctx *fasthttp.RequestCtx, f http.File) {
	...
	if err != nil {
		ctx.Logger().Printf("http: error reading directory: %v", err)
...
func ServeContent(ctx *fasthttp.RequestCtx, name string, modtime time.Time, content io.ReadSeeker) {
	...
	serveContent(ctx, name, modtime, sizeFunc, content)
}
...
func serveContent(ctx *fasthttp.RequestCtx, name string, modtime time.Time, sizeFunc func() (int64, error), content io.ReadSeeker) {
	...
	if ctype == "" || ctype == "text/plain; charset=utf-8" {
		...
		if ctype == "" {
			...
			if err != nil {
				ctx.Error("seeker can't seek", fasthttp.StatusInternalServerError)
				return
			}
		}
	...
	if size >= 0 {
		...
		if err != nil {
			...
			ctx.Error(err.Error(), fasthttp.StatusRequestedRangeNotSatisfiable)
			return
		}
		...
		case len(ranges) > 1:
			...
			ctx.SetContentType("multipart/byteranges; boundary=" + mw.Boundary())
			sendContent = pr
		...
		if len(ctx.Response.Header.Peek("Content-Encoding")) == 0 {
			ctx.Response.Header.SetContentLength(int(sendSize))
		}
	}
	...
	if !ctx.IsHead() {
		io.CopyN(ctx, sendContent, sendSize)
	}
}
...
func checkIfMatch(ctx *fasthttp.RequestCtx) condResult {
	im := b2s(ctx.Request.Header.Peek("If-Match"))
	if im == "" {
		return condNone
	}
	for {
		...
		if etagStrongMatch(etag, b2s(ctx.Request.Header.Peek("Etag"))) {
			return condTrue
		}
		im = remain
	}
...
func checkIfUnmodifiedSince(ctx *fasthttp.RequestCtx, modtime time.Time) condResult {
	ius := b2s(ctx.Request.Header.Peek("If-Unmodified-Since"))
...
func checkIfNoneMatch(ctx *fasthttp.RequestCtx) condResult {
	inm := b2s(ctx.Request.Header.Peek("If-None-Match"))
	if inm == "" {
		return condNone
	}
	buf := inm
	for {
		...
		if etagWeakMatch(etag, b2s(ctx.Response.Header.Peek("Etag"))) {
			return condFalse
		}
		buf = remain
	}
...
func checkIfModifiedSince(ctx *fasthttp.RequestCtx, modtime time.Time) condResult {
	...
	ims := b2s(ctx.Request.Header.Peek("If-Modified-Since"))
...
func checkIfRange(ctx *fasthttp.RequestCtx, modtime time.Time) condResult {
	...
	ir := b2s(ctx.Request.Header.Peek("If-Range"))
	if ir == "" {
		return condNone
	}
	etag, _ := scanETag(ir)
	if etag != "" {
		if etagStrongMatch(etag, b2s(ctx.Response.Header.Peek("Etag"))) {
			return condTrue
		} else {
			return condFalse
		}
...
func writeNotModified(ctx *fasthttp.RequestCtx) {
	...
	if len(ctx.Response.Header.Peek("Etag")) == 0 {
...
func serveFile(ctx *fasthttp.RequestCtx, fs http.FileSystem, name string, redirect bool) {
	const indexPage = "/index.html"

	// redirect .../index.html to .../
	// can't use Redirect() because that would make the path absolute,
	// which would be a problem running under StripPrefix
	if strings.HasSuffix(b2s(ctx.Path()), indexPage) {
		localRedirect(ctx, "./")
		return
	}
...
func localRedirect(ctx *fasthttp.RequestCtx, newPath string) {
	if q := b2s(ctx.URI().QueryString()); q != "" {
		newPath += "?" + q
	}
...
func ServeFile(ctx *fasthttp.RequestCtx, name string) {
	if containsDotDot(b2s(ctx.Path())) {
		// Too many programs use r.URL.Path to construct the argument to
		// serveFile. Reject the request under the assumption that happened
		// here and ".." may not be wanted.
		// Note that name might not contain "..", for example if code (still
		// incorrectly) used filepath.Join(myDir, r.URL.Path).
		ctx.Error("invalid URL path", fasthttp.StatusBadRequest)
		return
	}
	dir, file := filepath.Split(name)
	serveFile(ctx, http.Dir(dir), file, false)
}
...
//
func FileServer(root http.FileSystem) fasthttp.RequestHandler {
	return func(ctx *fasthttp.RequestCtx) {
		upath := b2s(ctx.Path())
		if !strings.HasPrefix(upath, "/") {
			upath = "/" + upath
			ctx.Request.URI().SetPath(upath)
		}
		serveFile(ctx, root, path.Clean(upath), true)
	}
}
